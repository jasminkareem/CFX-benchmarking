...
def get_data_for_feature(dist_data, target_class, feature_map_num):
	...
	#print(f"Shape of data: {data.shape}") # (5964, 1, 10) for alibi cnn --> wrong! (5964, 256) --> right!
	#print("feature_map_num:", feature_map_num) # should go to 128, 1024, 256, etc.
	#print(f"Shape of transposed data: {transposed_data.shape}")
	# When feature_map_num is equal to or larger than 128, we have a problem which leads to the IndexError --> check if you have the right pickle file
...
def acquire_feature_probabilities(target_class, cnn, original_query_img=None, alpha=0.05, picklefile=None):
	#query_features = cnn(original_query_img)[0] # This is where the problem is, this is not 128 but 200 or 1024
	...
	#print("len query features:", len(query_features))
	...
	#print('q_features:', query_features.detach().numpy().shape)
	#print('d_weights:', digit_weights.detach().numpy().shape)
...
def modifying_exceptional_features(df, target_class, query_activations):
	...
	ideal_xp = ideal_xp[0] # remove if needed
...
def filter_df_of_exceptional_noise(df, target_class, cnn, alpha=0.05):
	...
	# Again change cnn.layer4.weight[target_class] to whatever suits your network architecture
	digit_weights = cnn.classifier.weight[target_class] # this is where i get the TypeError: 'Linear object not subscriptable'. Fixed by removing '[0]'
