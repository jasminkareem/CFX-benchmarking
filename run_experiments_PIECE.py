...
def main():
    ...
    if dataset == 'mnist': 
        train_loader, test_loader = load_dataloaders(location=os.path.join(save_path, 'mnist'))
    ...
    for sample_num in range(n_samples):
        ...
        image_tensor = torch.from_numpy(original_query_img.detach().numpy())
        outfile = path_explanations + 'original/instance_' + str(sample_num) + '.pt'
        new_outfile = os.path.join(save_path, outfile)
        pathlib.Path(os.path.join(save_path, path_explanations + 'original')).mkdir(parents=True, exist_ok=True)
        ...
        for target_class in target_classes:
            ...
            z_path = latent_file_path + str(sample_num) + ".pt"
            z = torch.load(os.path.join(save_path, z_path))
            ...
            for name in ['PIECE', 'Min-Edit', 'C-Min-Edit']:  # 'CEM', 'Proto-CF']:
                ...
                image_tensor_I_e = torch.from_numpy(I_e.detach().numpy())
                outfile = path_explanations + name + '/instance_' + str(sample_num) + '_target_' + str(target_class) + '.pt'
                pathlib.Path(os.path.join(save_path, path_explanations + name)).mkdir(parents=True, exist_ok=True)
                torch.save(image_tensor_I_e, os.path.join(save_path, outfile))
    ...
    csv_path = path_explanations +'output_PIECE_'+ dataset +'_data.csv'
    expt1_data.to_csv(os.path.join(save_path, csv_path), index=False) 
